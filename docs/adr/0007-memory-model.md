...
4. **Whole-memory live-out is auto-derived.** A new helper `validation::live_out::touches_memory(&[Instruction]) -> bool` flags any sequence containing a memory op. `EquivalenceConfig.memory_live` defaults to false but is force-set to `true` inside `check_equivalence_with_config` whenever `touches_memory()` returns true on either sequence. There is no CLI knob this PR — the user does not have to remember `;mem`. Search algorithms (enumerative, stochastic, symbolic) continue to pin `with_memory(true)` as a conservative whole-memory contract; unlike NZCV liveness after issue #138, memory liveness is not derived from surrounding context.
