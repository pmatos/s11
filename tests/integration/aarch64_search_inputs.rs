...
//! (#754) these functions lived in the binary crate and were unreachable from
//! an integration test — the only way to exercise the register-narrowing veto
//! or the single-basic-block gate was to drive a whole optimization run.
//! Testing `s11::aarch64_search_inputs` from outside the crate here mirrors
//! the coverage style used for `s11::x86_search_inputs` (#752).
//!
//! Expected values are the known-good literals from the seam's own unit tests.

use s11::aarch64_search_inputs::{
    live_out_for_optimization_prefix, registers_from_target, validate_basic_block,
};
use s11::ir::{
    Condition, Instruction, LabelId, Operand, Register, VectorArrangement, VectorRegister,
};
use s11::semantics::RegisterSet;
...
#[test]
fn registers_from_target_expose_base_gprs_and_target_vectors() {
    let target = [
