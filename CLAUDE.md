...
The regression test `convert_capstone_op_handles_all_supported_aarch64_mnemonics` in `src/capstone_bridge.rs` pins one canonical operand string per supported mnemonic — extend it whenever you add an opcode so a future Capstone-syntax regression on that mnemonic fails loudly.
...
Issues live in GitHub Issues (pmatos/s11), managed via the `gh` CLI. See `docs/agents/issue-tracker.md`.

### Triage labels

Canonical roles map to this repo's existing labels where they exist (`ready-for-human` → `human-needed`); `needs-triage`/`needs-info` are new. See `docs/agents/triage-labels.md`.
