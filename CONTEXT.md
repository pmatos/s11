...
### LLM run accounting
The bookkeeping an LLM-assisted search run keeps as it drives Codex: the three accumulators — `SearchStatistics`, the `LlmTimings` per-phase timing breakdown, and the [unsupported-mnemonic ledger](#unsupported-mnemonic-ledger) — together with the rules for which loop event bumps which counter. The rules are subtle: a Codex `Ok` counts as an *evaluated* candidate but an IO/exit error does not; the SMT counters key off the verification metrics (whether the solver was reached) rather than the classified outcome, so an equivalence-unknown that still reached Z3 is counted; and formula-byte totals accumulate only on solver-reaching verifications. This concern lives behind the pure `RunAccounting` seam in `src/search/llm/accounting.rs`, which the `LlmSearch::search` loop feeds one `RunEvent` per accountable moment and drains once with `finish`; the loop itself keeps only control flow and verbose logging. The seam exists so the counting rules are unit-testable without a `FakeCodex` subprocess.
